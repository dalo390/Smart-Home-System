...
def getDays(numDays):
    ...
    date_list = [base - datetime.timedelta(days=x) for x in range(0, numDays, 1 if numDays > 0 else -1)]
    return date_list
...
def runHVAC(
    ...
    closed = 24
    temp_change = (
        abs(target_temp - external_temp) / 10 * (door + closed) * 2
    )
...
def runElectric(device: str, day: int) -> float:
    """
...
def runDay(
    day: int, weeklies_opt: tuple, avg_temp: float, target_temp: float = 70  # assuming 70F is the target temp
